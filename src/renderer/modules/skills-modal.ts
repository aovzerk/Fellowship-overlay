(() => {
  ...
  function buildAbilityOption(classId: number, ability: SkillCatalogAbility, selected: boolean): string {
    ...
    return `
      <label class="skill-option" title="${escapeHtml(ability.tooltip || ability.name || '')}">
  ...
  function buildDraggableOrderItem(token: string, classId: number, abilitiesById: Map<string, SkillCatalogAbility>, t: (key: string) => string): string {
    ...
    const tooltip = isRelics ? t('relics') : (ability?.tooltip || ability?.name || t('unknown'));

    return `
      <div class="order-item" draggable="true" data-order-class-id="${classId}" data-order-token="${escapeHtml(token)}" title="${escapeHtml(tooltip)}">
  ...
  function bindOrderDragAndDrop({ latestData, renderPlayers, saveSkillSelections, selectedSkillsByClass, skillsCatalogEl }: RenderSkillsModalArgs): void {
    let draggedClassId = '';
    let draggedToken = '';
    ...
    skillsCatalogEl.querySelectorAll<HTMLElement>('.order-item').forEach((item) => {
      ...
      item.addEventListener('drop', (event: DragEvent) => {
        ...
        saveSkillSelections();
        renderSkillsModal({ latestData, renderPlayers, saveSkillSelections, selectedSkillsByClass, skillCatalog: { classes: [] }, skillsCatalogEl, t: () => '' });
      });
    });
  }
